  ...
  return (
      ...
      >
        <div className='w-full max-w-7xl mx-auto px-4 '>
          {/* Toggle buttons - Now using theme-aware colors */}
          <motion.div
          ...
          >
            <div className='relative inline-flex items-center rounded-full border border-ring/10 p-1 bg-background'>
              <button
                  onClick={() => handlePlanChange(true)}
                  className={cn(
                      'relative rounded-sm px-3 py-1.5 text-sm font-medium outline-2 outline-sky-400 transition focus-visible:outline',
                      isMonthly
                  ...
                  <div>
                    ...
                    <div className='flex items-baseline gap-x-2 mb-6'>
                      <AnimatePresence mode='wait'>
                        {/* Modificada la visualización de precios */}
                        {plan.price === 0 ? (
                            <motion.span
                                key="free"
                            ...
                            <div className='flex flex-col'>
                              <div className='flex items-baseline'>
                                <motion.span
                                    ...
                                    exit={{ opacity: 0, y: -10 }}
                                    transition={{ duration: 0.2 }}
                                    className='text-lg font-medium tracking-tight text-red-500 line-through decoration-2'
                                >
                                  ${getOriginalYearlyPrice(plan.price).toFixed(2)}
                                </motion.span>
                                <span className='text-xs text-red-500 line-through ml-1'>/yr</span>
                              </div>
                              <div className='flex items-baseline'>
                                <motion.span
                                    key="yearly-discounted"
                                    initial={{ opacity: 0, y: 10 }}
                                    animate={{ opacity: 1, y: 0 }}
                                    exit={{ opacity: 0, y: -10 }}
                                    transition={{ duration: 0.2, delay: 0.1 }}
                                    className='text-4xl font-bold tracking-tight text-blue-500'
                                >
                                  ${getYearlyPrice(plan.price).toFixed(2)}
                                </motion.span>
                                <span className='text-sm text-blue-500 ml-1'>/yr</span>
                              </div>
                            </div>
                        )}
                      </AnimatePresence>
